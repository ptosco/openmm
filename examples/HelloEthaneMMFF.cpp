...
//                            FORCE FIELD DATA
// For this example we're using a tiny subset of the MMFF94 force field.
...
// MMFF reduces electrostatic forces between 1-4 bonded atoms.
...
struct MMFFAtomType {
    ...
    char vdwDA;
} mmffAtomType[] = {/*0 H*/ 1.008, 0.000, 2.96985, 0.30225, 0.3125, '-',
...
struct MMFFBondType {
    double nominalLengthInAngstroms, stiffnessInKcalPerAngstrom2;
    bool   canConstrain;
} mmffBondType[] = {/*0 CC*/1.508, 612.865, false,
...
struct MMFFAngleType {
    double nominalAngleInDegrees, stiffnessInKcalPerRadian2;
    bool isLinear;
} mmffAngleType[] = {/*0 HCC*/110.549, 91.5411, false,
                 /*1 HCH*/108.836, 74.2692, false};
const int HCC = 0, HCH = 1;

struct MMFFStretchBendType {
    double nominalLengthInAngstroms1, nominalLengthInAngstroms2, nominalAngleInDegrees,
        stiffnessInKcalPerAngstromPerRadian1, stiffnessInKcalPerAngstromPerRadian2;
                    ...
                    std::string&        platformName) 
{
    ...
    OpenMM::MMFFNonbondedForce&     nonbond      = *new OpenMM::MMFFNonbondedForce();
    OpenMM::MMFFBondForce&          bondStretch  = *new OpenMM::MMFFBondForce();
    OpenMM::MMFFAngleForce&         angleBend    = *new OpenMM::MMFFAngleForce();
    OpenMM::MMFFStretchBendForce&   angleStbn    = *new OpenMM::MMFFStretchBendForce();
    OpenMM::MMFFTorsionForce&       angleTorsion = *new OpenMM::MMFFTorsionForce();
    system.addForce(&nonbond);
    system.addForce(&bondStretch);
    system.addForce(&angleBend);
    system.addForce(&angleStbn);
    system.addForce(&angleTorsion);
    ...
    for (int n=0; atoms[n].type != EndOfList; ++n) {
        const MMFFAtomType& atype = mmffAtomType[atoms[n].type];
        system.addParticle(atype.mass);
        nonbond.addParticle(atype.charge,
                            atype.sigma * OpenMM::NmPerAngstrom,
                            atype.G_t_alpha, atype.alpha_d_N, atype.vdwDA);
    ...
    for (int i=0; bonds[i].type != EndOfList; ++i) {
        ...
        const MMFFBondType& bond = mmffBondType[bonds[i].type];
    ...
    // Create the 1-2-3 angle bend terms.
    for (int i=0; angles[i].type != EndOfList; ++i) {
        const int*           atom  = angles[i].atoms;
        const MMFFAngleType& angle = mmffAngleType[angles[i].type];

        angleBend.addAngle(atom[0],atom[1],atom[2],
                          angle.nominalAngleInDegrees     * OpenMM::RadiansPerDegree,
                          angle.stiffnessInKcalPerRadian2 * OpenMM::KJPerKcal,
                          angle.isLinear);
    }

    // Create the 1-2-3 stretch-bend terms.
    for (int i=0; angles[i].type != EndOfList; ++i) {
        const int*                 atom  = angles[i].atoms;
        const MMFFStretchBendType& angle = mmffStretchBendType[angles[i].type];

        angleStbn.addStretchBend(atom[0],atom[1],atom[2],
                          angle.nominalLengthInAngstroms1 * OpenMM::NmPerAngstrom,
                          angle.nominalLengthInAngstroms2 * OpenMM::NmPerAngstrom,
                          angle.nominalAngleInDegrees     * OpenMM::RadiansPerDegree,
                          angle.stiffnessInKcalPerAngstromPerRadian1 * OpenMM::NmPerAngstrom * OpenMM::KJPerKcal,
                          angle.stiffnessInKcalPerAngstromPerRadian2 * OpenMM::NmPerAngstrom * OpenMM::KJPerKcal);
    }

    // Create the 1-2-3-4 bond torsion (dihedral) terms.
    for (int i=0; torsions[i].type != EndOfList; ++i) {
        const int*         atom = torsions[i].atoms;
        const MMFFTorsionType& torsion = mmffTorsionType[torsions[i].type];
        angleTorsion.addTorsion(atom[0],atom[1],atom[2],atom[3], 
                          torsion.V1inKcal * OpenMM::KJPerKcal,
                          torsion.V2inKcal * OpenMM::KJPerKcal,
                          torsion.V3inKcal * OpenMM::KJPerKcal);
    }
