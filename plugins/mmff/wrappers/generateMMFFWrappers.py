...
class CHeaderGenerator(WrapperGenerator):
    ...
    def writeMethods(self, classNode):
        ...
        if not ('abstract' in classNode.attrib and classNode.attrib['abstract'] == 'yes'):
            ...
            for methodNode in methodList:
                ...
                if methodName == shortClassName:
                    ...
                    self.out.write("extern OPENMM_EXPORT_MMFF %s* %s_create%s(" % (typeName, typeName, suffix))
                    self.writeArguments(methodNode, False)
                    self.out.write(");\n")
    
        # Write destructor
        self.out.write("extern OPENMM_EXPORT_MMFF void %s_destroy(%s* target);\n" % (typeName, typeName))
        ...
        for methodNode in methodList:
            ...
            if isConstMethod and any(methodNames[m] == methodName and m.attrib['const'] == 'no' for m in methodList):
                ...
                continue
            returnType = self.getType(getText("type", methodNode))
            self.out.write("extern OPENMM_EXPORT_MMFF %s %s_%s(" % (returnType, typeName, methodName))
    ...
    def writeOutput(self):
        print("""
#ifndef MMFF_OPENMM_CWRAPPER_H_
#define MMFF_OPENMM_CWRAPPER_H_

#ifndef OPENMM_EXPORT_MMFF
#define OPENMM_EXPORT_MMFF
#endif""", file=self.out)
...
OPENMM_EXPORT_MMFF OpenMM_3D_DoubleArray* OpenMM_3D_DoubleArray_create(int size1, int size2, int size3);
OPENMM_EXPORT_MMFF void OpenMM_3D_DoubleArray_set(OpenMM_3D_DoubleArray* array, int index1, int index2, OpenMM_DoubleArray* values);
OPENMM_EXPORT_MMFF void OpenMM_3D_DoubleArray_destroy(OpenMM_3D_DoubleArray* array);""", file=self.out)
...
#endif /*MMFF_OPENMM_CWRAPPER_H_*/""", file=self.out)
...
class CSourceGenerator(WrapperGenerator):
    ...
    def writeMethods(self, classNode):
            ...
            for methodNode in methodList:
                ...
                if methodName == shortClassName:
                    ...
                    if numConstructors == 1:
                        ...
                        suffix = "_%d" % numConstructors
                    self.out.write("OPENMM_EXPORT_MMFF %s* %s_create%s(" % (typeName, typeName, suffix))
                    self.writeArguments(methodNode, False)
                    self.out.write(") {\n")
        ...
        # Write destructor
        self.out.write("OPENMM_EXPORT_MMFF void %s_destroy(%s* target) {\n" % (typeName, typeName))
        ...
        for methodNode in methodList:
            ...
            self.out.write("OPENMM_EXPORT_MMFF %s %s_%s(" % (returnType, typeName, methodName))
            isInstanceMethod = (methodNode.attrib['static'] != 'yes')
            if isInstanceMethod:
                if isConstMethod:
                    self.out.write('const ')
...
/* OpenMM_2D_IntArray */
OPENMM_EXPORT_MMFF OpenMM_2D_IntArray* OpenMM_2D_IntArray_create(int size) {
    return reinterpret_cast<OpenMM_2D_IntArray*>(new vector<vector<int> >(size));
}
OPENMM_EXPORT_MMFF void OpenMM_2D_IntArray_destroy(OpenMM_2D_IntArray* array) {
    delete reinterpret_cast<vector<vector<int> >*>(array);
}
OPENMM_EXPORT_MMFF int OpenMM_2D_IntArray_getSize(const OpenMM_2D_IntArray* array) {
    return reinterpret_cast<const vector<vector<int> >*>(array)->size();
}
OPENMM_EXPORT_MMFF void OpenMM_2D_IntArray_resize(OpenMM_2D_IntArray* array, int size) {
    reinterpret_cast<vector<vector<int> >*>(array)->resize(size);
}
OPENMM_EXPORT_MMFF void OpenMM_2D_IntArray_append(OpenMM_2D_IntArray* array, int index1, int value) {
...
OPENMM_EXPORT_MMFF void OpenMM_2D_IntArray_set(OpenMM_2D_IntArray* array, int index1, int index2, int value) {
    vector<vector<int> >* array2DInt = reinterpret_cast<vector<vector<int> >*>(array);
    if (array2DInt->size() <= index1) {
        array2DInt->resize(index1+1);
    }
...
OPENMM_EXPORT_MMFF void OpenMM_2D_IntArray_get(const OpenMM_2D_IntArray* array, int index1, int index2, int* value) {
...
OPENMM_EXPORT_MMFF void OpenMM_3D_DoubleArray_destroy(OpenMM_3D_DoubleArray* array) {
...
class FortranSourceGenerator(WrapperGenerator):
    ...
    def writeOneConstructor(self, classNode, methodNode, functionName, wrapperFunctionName):
        ...
        shortClassName = stripOpenMMPrefix(className)
        typeName = convertOpenMMPrefix(className)
        self.out.write("OPENMM_EXPORT_MMFF void %s(%s*& result" % (wrapperFunctionName, typeName))
        self.writeArguments(methodNode, True)
        self.out.write(") {\n")
    ...
    def writeOneDestructor(self, typeName, wrapperFunctionName):
        self.out.write("OPENMM_EXPORT_MMFF void %s(%s*& destroy) {\n" % (wrapperFunctionName, typeName))
    ...
    def writeOneMethod(self, classNode, methodNode, methodName, wrapperFunctionName):
        ...
        if methodType in self.classesByShortName:
            methodType = self.classesByShortName[methodType]
        self.out.write("OPENMM_EXPORT_MMFF ")
        if hasReturnValue:
...
builder = CHeaderGenerator(inputDirname, open(os.path.join(sys.argv[2], 'MMFFOpenMMCWrapper.h'), 'w'))
builder.writeOutput()
builder = CSourceGenerator(inputDirname, open(os.path.join(sys.argv[2], 'MMFFOpenMMCWrapper.cpp'), 'w'))
builder.writeOutput()
builder = FortranHeaderGenerator(inputDirname, open(os.path.join(sys.argv[2], 'MMFFOpenMMFortranModule.f90'), 'w'))
builder.writeOutput()
builder = FortranSourceGenerator(inputDirname, open(os.path.join(sys.argv[2], 'MMFFOpenMMFortranWrapper.cpp'), 'w'))
builder.writeOutput()
